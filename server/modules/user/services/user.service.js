...
export class UserService {
    ...
    async bulkCreateUsers(csvBuffer) {
        ...
        for (let i = 0; i < rows.length; i++) {
            ...
            const validation = UserUtils.validateCSVRow(row, i);
        ...
        if (validUsers.length > 0) {
            try {
                ...
                result.created = created.map((u) =>
                    UserUtils.sanitizeUserData(u)
                );
            } catch (error) {
